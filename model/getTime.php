...
if (isset($_SESSION["username"])) {
    global $pdo;
    $statement = $pdo->prepare("SELECT testTime FROM  db_name.tests WHERE  userID = (SELECT users.id FROM db_name.users WHERE username = ?)
    ORDER  BY id DESC LIMIT  1");
    $statement->bindValue(1, $_SESSION["username"], PDO::PARAM_STR);
    $statement->execute();
    $results = $statement->fetchAll(PDO::FETCH_CLASS, "tests");
    echo json_encode($results[0]->testtime);
}
if (!isset($_SESSION["username"])) {
